!function() {
  ...
  d3.svg.brush = function() {
    var event = d3_eventDispatch(brush, "brushstart", "brush", "brushend", "mouseover", "mousemove", "mouseout"), x = null, y = null, xExtent = [ 0, 0 ], yExtent = [ 0, 0 ], xExtentDomain, yExtentDomain, xClamp = true, yClamp = true, resizes = d3_svg_brushResizes[0];
    function brush(g) {
      g.each(function() {
        var event_ = event.of(this, arguments);
        var g = d3.select(this).style("pointer-events", "all").style("-webkit-tap-highlight-color", "rgba(0,0,0,0)").on("mousedown.brush", brushstart).on("touchstart.brush", brushstart).on("mouseover.brush", function() {
          event_({
            type: "mouseover"
          });
        }).on("mouseout.brush", function() {
          event_({
            type: "mouseout"
          });
